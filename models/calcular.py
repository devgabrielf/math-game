...
class Calcular:
    ...
    def checar_resultado(self: object, resposta: int) -> bool:
        ...
        if self.resultado == resposta:
            print(f'Você acertou :) (+{self.dificuldade} ponto(s))')
            certo = True
        else:
            print('Você errou :(')
            print(f'Resposta correta: {self.valor1} {self._op_simbolo} {self.valor2} = {self.resultado}')
        return certo

    def mostrar_operacao(self: object) -> None:
        print(f'{self.valor1} {self._op_simbolo} {self.valor2} = ', end='')
